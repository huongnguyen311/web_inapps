// Image Option B: Stacked layers + red dot corner
import type { CaseStudyDetail } from "@/data/caseStudyDetailMock";
type Props = Pick<CaseStudyDetail, "teamStructure" | "teamImage">;

export default function ProcessSectionB_ImgB({ teamStructure, teamImage }: Props) {
      ...
      <div className="max-w-[1200px] mx-auto relative" style={{ zIndex: 1 }}>
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-[64px] items-start">
          ...
          <div>
            ...
            {/* Image B: stacked layers */}
            {teamImage && (
              <div className="relative" style={{ height: 280, paddingBottom: 14, paddingRight: 14 }}>
                {/* Layer 3: red ghost */}
                <div style={{ position: "absolute", top: 14, right: 0, bottom: 0, left: 14, borderRadius: 14, background: "rgba(255,73,41,0.12)", border: "1px solid rgba(255,73,41,0.25)" }} />
                {/* Layer 2: dark ghost */}
                <div style={{ position: "absolute", top: 7, right: 7, bottom: 7, left: 7, borderRadius: 14, background: "rgba(0,0,0,0.04)" }} />
                {/* Layer 1: image */}
                <div style={{ position: "absolute", top: 0, right: 14, bottom: 14, left: 0, borderRadius: 14, overflow: "hidden", boxShadow: "0 16px 40px rgba(0,0,0,0.14)", zIndex: 1 }}>
                ...
                {/* Red dot corner */}
                <div style={{ position: "absolute", top: -8, left: -8, width: 20, height: 20, borderRadius: "50%", background: "#FF4929", zIndex: 2, boxShadow: "0 2px 8px rgba(255,73,41,0.4)" }} />
              </div>
            )}
          </div>
